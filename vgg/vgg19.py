# This program trains a VGG19 architecture to measure the length
...
run_id = datetime.now().strftime("VGG19 %Y_%m_%d T %H-%M-%S")
...
def make_model():
    """
    Creates a VGG19 model and compiles it with the Adam algorithm
    ...
    x = tf.keras.layers.Dense(2048, activation="elu", kernel_initializer=tf.keras.initializers.GlorotUniform(seed=rnd_seed))(x)   # 4096
    x = tf.keras.layers.Dropout(0.5, seed=rnd_seed)(x)

    x = tf.keras.layers.Dense(2048, activation="elu", kernel_initializer=tf.keras.initializers.GlorotUniform(seed=rnd_seed))(x)
    x = tf.keras.layers.Dropout(0.5, seed=rnd_seed)(x)
    
    output_layer = tf.keras.layers.Dense(1, kernel_initializer=tf.keras.initializers.GlorotUniform(seed=rnd_seed))(x)
...
model.save(os.path.join(logdir, "VGG19"))
