...
def init_inception_v4(lr, hidden, drop):
    lr = 1 / np.power(10, lr)

    base_inception = inception_v4(weights="imagenet", include_top=False, num_classes=1000, dropout_keep_prob=0.5)
    ...
    inception_v4_model = tf.keras.Model(inputs=base_inception.input, outputs=output_layer)

    inception_v4_model.compile(tf.keras.optimizers.Adam(lr=lr), tf.keras.losses.MeanSquaredError(), ["mae", "accuracy"])
    return inception_v4_model


def init_inception_v3(lr, hidden, drop):
    lr = 1 / np.power(10, lr)
    ...
    inception_v3_model.compile(tf.keras.optimizers.Adam(lr=lr), tf.keras.losses.MeanSquaredError(), ["mae", "accuracy"])
...
def evaluate_func(**kwargs):
    ...
    model = tf.keras.wrappers.scikit_learn.KerasRegressor(build_fn=init_inception_v4, epochs=2)
    model.set_params(**kwargs)

    x_train, x_test, y_train, y_test = train_test_split(x_data, y_data, test_size=0.33, random_state=42)
    x_train = np.random.random((10,299,299,3))
    y_test = np.random.random((5,299,299,3))
