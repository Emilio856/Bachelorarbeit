# This program trains an EfficientNet B1 architecture to measure the length
...
run_id = datetime.now().strftime("EffNetB1 %Y_%m_%d T %H-%M-%S")
...
def make_model():
    """
    Creates an EfficientNet B1 model and compiles it with the Adam algorithm
    and the Mean Squared Error.

    Returns:
      An EfficientNet B1 model.
    """
    base_b1 = tf.keras.applications.EfficientNetB1(input_shape=(450, 450, 3),
                                                   include_top=False, weights="imagenet", pooling="avg")
    x = base_b1.output
    x = tf.keras.layers.Dropout(0.5, seed=rnd_seeds)(x)
...
model.save(os.path.join(logdir, "EffNetB1"))
