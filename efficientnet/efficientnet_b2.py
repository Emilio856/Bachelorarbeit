# This program trains an EfficientNet B2 architecture to measure the length
...
run_id = datetime.now().strftime("EffNetB2 %Y_%m_%d T %H-%M-%S")
...
def make_model():
    """
    Creates an EfficientNet B2 model and compiles it with the Adam algorithm
    and the Mean Squared Error.

    Returns:
      An EfficientNet B2 model.
    """
    base_b2 = tf.keras.applications.EfficientNetB2(input_shape=(450, 450, 3),
    ...
    b1_model = tf.keras.Model(inputs=base_b2.input, outputs=output_layer)
...
model.save(os.path.join(logdir, "EffNetB2"))
