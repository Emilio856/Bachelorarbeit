...
from mobilenet_v3 import MobileNetV3Large, MobileNetV3Small, MobileNetV3
...
def init_mn_v3_small(lr, hidden, drop):
    lr = 1 / np.power(10, lr)

    base_mn = MobileNetV3Small(input_shape=(img_height, img_width, channels), include_top=False, weights="imagenet")
    x = base_mn.output
    ...
    mobilenet_model = tf.keras.Model(inputs=base_mn.input, outputs=output_layer)

    mobilenet_model.compile(tf.keras.optimizers.Adam(lr=lr), tf.keras.losses.MeanSquaredError(), ["mae", "accuracy"])
    return mobilenet_model


def init_mn_v3_large(lr, hidden, drop):
    lr = 1 / np.power(10, lr)

    base_mn = MobileNetV3Large(input_shape=(img_height, img_width, channels), include_top=False, weights="imagenet")
    x = base_mn.output
    x = tf.keras.layers.Flatten()(x)
    x = tf.keras.layers.Dropout(drop)(x)
...
def evaluate_func(**kwargs):
    ...
    model = tf.keras.wrappers.scikit_learn.KerasRegressor(build_fn=init_mn_v3_small,epochs=2)
