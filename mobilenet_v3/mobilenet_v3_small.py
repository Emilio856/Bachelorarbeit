# This program trains a MobileNetV3 small architecture to measure the length
...
run_id = datetime.now().strftime("MNV3 small %Y_%m_%d T %H-%M-%S")
...
def make_model():
    """
    Creates a MobileNetV3 small model and compiles it with the Adam algorithm
    and the Mean Squared Error.

    Returns:
      A MobileNetV3 small model.
    """
    base_mn = MobileNetV3Small(
    ...
    output_layer = tf.keras.layers.Dense(
        1,
        kernel_initializer=tf.keras.initializers.GlorotNormal(seed=rnd_seed)
    )(x)
...
model.save(os.path.join(logdir, "MNV3 small"))
