# This program trains a ResNet101V2 architecture to measure the length
...
run_id = datetime.now().strftime("ResNetV2 101 %Y_%m_%d T %H-%M-%S")
...
def make_model():
    """
    Creates a ResNet101V2 model and compiles it with the Adam algorithm
    and the Mean Squared Error.

    Returns:
      A ResNet101V2 model.
    """
    base_resnet = tf.keras.applications.ResNet101V2(input_shape=(450, 450, 3),
    ...
    resnet_model = tf.keras.Model(inputs=base_resnet.input, outputs=output_layer)
...
model.save(os.path.join(logdir, "ResNetV2 101"))
